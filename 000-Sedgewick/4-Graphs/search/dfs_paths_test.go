...
func TestPathToVertex(t *testing.T) {
	...
	t.Run("star graph - leaf to leaf", func(t *testing.T) {
		...
		path := PathToVertex(g, 1, 3)

		expected := []int{1, 0, 3}
		assert.Equal(t, expected, path, "Path between leaves should go through center")
	})
	...
	t.Run("linear chain - backward", func(t *testing.T) {
		...
		path := PathToVertex(g, 4, 0)

		expected := []int{4, 3, 2, 1, 0}
		assert.Equal(t, expected, path, "Should find reverse chain path")
	})
