...
func TestReachableVertices(t *testing.T) {
	...
	t.Run("partially connected graph", func(t *testing.T) {
		...
		marked2, count2 := ReachableVertices(g, 2)
	...
	t.Run("linear chain", func(t *testing.T) {
		...
		assert.Equal(t, expected, marked, "All vertices should be reachable in linear chain")
		assert.Equal(t, 5, count, "Count should be 5")

		// Test from middle
		marked2, count2 := ReachableVertices(g, 2)
		expected2 := []bool{true, true, true, true, true}
		assert.Equal(t, expected2, marked2, "All vertices should be reachable from middle of chain")
		assert.Equal(t, 5, count2, "Count should be 5")
	})
	...
	t.Run("star graph", func(t *testing.T) {
		...
		// Test from leaf
		marked2, count2 := ReachableVertices(g, 1)
		expected2 := []bool{true, true, true, true}
		assert.Equal(t, expected2, marked2, "All vertices should be reachable from leaf in undirected graph")
		assert.Equal(t, 4, count2, "Count should be 4")
	})
